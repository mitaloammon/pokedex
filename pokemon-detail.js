const API_BASE = "https://corsproxy.io/?https://pokeapi.co/api/v2";
...
async function loadPokemon(id) {
    try{
        const [pokemon, pokemonSpecies] = await Promise.all([
            fetch(`${API_BASE}/pokemon/${id}`).then(res => res.json()),
            fetch(`${API_BASE}/pokemon-species/${id}`).then(res => res.json()),
        ]);
...
function displayPokemonDetails(pokemon) {
    ...
    ".pokemon-detail-wrap .pokemon-detail .weight"
    ).textContent = `${weight / 10} kg`;

    document.querySelector(
    ".pokemon-detail-wrap .pokemon-detail .height"
